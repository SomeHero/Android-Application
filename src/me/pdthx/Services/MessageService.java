...
public class MessageService {
	...
	public ArrayList<MessageResponse> GetMessages(
		...
		try {
			...
			HttpGet request = new HttpGet(ROOTURL + String.format(PAYSTREAMMESSAGES_URL, messageRequest.UserId) + "?apiKey=" + APIKEY);
		...
		if (entity != null) {
			...
			try{
				...
				for(int i=0;i < messages.length();i++){ 
					...
					try {
						formattedDate = sdf.parse(jsonResult.getString("createDate"));
					...
					messageResponse.MessageStatus = jsonResult.getString("messageStatus");
